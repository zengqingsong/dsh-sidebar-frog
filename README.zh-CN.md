...
- **文件树。** 支持展开、折叠、过滤、键盘操作和按目录刷新（工具栏上的刷新会保留你的展开状态，`Shift` + 点击才是整体重载）。右键菜单提供复制路径、复制相对路径、`@引用`、仅刷新此目录、展开/折叠全部，以及删除文件、删除文件夹（带二次确认，每次删除都会被宿主圈定在工作区内）。展开状态在刷新后依然保留。
...
- **每次推送，Linux 与 Windows 双平台**：守卫套件 178 条断言，外加 28 条在真实 Chrome 里驱动弹出页的用例。

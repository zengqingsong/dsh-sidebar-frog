...
- 面向 `web` profile 的 DSH `0.1.7-rc.1` 开发。安装过程不执行任何构建脚本。
...
- **弹出页。** `/dsh-sidebar-frog` 是一个独立的两栏页面：左边预览，右边是文件树或台账，中间的分隔条可以拖动。它就是个普通标签页，能拖到另一块屏幕上，并通过 `storage` 事件和侧边栏保持同步。弹出页上的 `@` 按钮会把引用直接写进主窗口的输入框；主窗口不在了才退回剪贴板。**它和侧边栏都默认落在文件树上**：文件树是“怎么找到文件”，台账是“智能体改了什么”——后者是之后才问的问题。把「文件树」开关关掉时，两边都会退回台账（没有树可开了）。

...
| ↔️ | 工作区文件树             | 系统自带的「文件」标签页保持原样（含 0.1.7 的实时监听与自动刷新），本插件的文件树是它旁边的「文件树」标签：`@引用`、右键菜单、新建/删除、按目录刷新都在。`@引用` 同时也挂在系统文档预览的工具栏上，所以用系统那棵树一样能引用。                                                                                                                                                                                   |
...
| 文件树                       | 开               | 在浮动面板的标签条上显示文件树。原生侧边栏形态不提供这个开关：本插件的**文件树**标签必须一直显示。系统的**文件**标签是另一棵，不受影响。                                                                                                                             |
| 用系统右侧边栏承载面板       | 开               | 把每个视图都注册成系统右侧边栏自己的标签页，展开/收起、全屏、拖宽、标签条都由系统提供。系统的「文件」标签不受影响，始终是系统自带的那棵树。关掉之后面板退回浮动形态。刷新页面后生效。                                                                                |

...
- 面向 `web` profile 的 DSH `0.1.7-rc.2` 开发。安装过程不执行任何构建脚本。
...
| ↔️ | 工作区文件树             | 右侧边栏只有**一棵**文件树：本插件的树放在系统「文件」这个位置，`@引用`、右键菜单、新建/删除、按目录刷新与 A/M 改动字母都在——不再有两棵画着同一个工作区的树。想让系统那棵（带实时目录监听）也回来，打开设置里的「显示系统的文件树」，它就成了旁边的「文件树」标签，两棵都在。`@引用` 同时也挂在系统文档预览的工具栏上，所以用系统那棵树一样能引用。                                                                                                                                                                                   |
...
装完重启 DSH 服务，再强制刷新浏览器（`Ctrl/Cmd+Shift+R`）。右侧边栏会在系统自带的**文件**标签页旁边多出五个标签页：**文件树**、**产物**、**任务**、**用量**、**Git**；左栏底部还会多一个常驻的**文件树**按钮，新建的会话也从这里进侧边栏。
...
- **弹出页。** `/dsh-sidebar-frog` 是一个独立的两栏页面：左边预览，右边是文件树或台账，中间的分隔条可以拖动。它就是个普通标签页，能拖到另一块屏幕上，并通过 `storage` 事件和侧边栏保持同步。**顶部只有两行、共 60px**：一行窗口标题（标题 + 连接状态点，绿点=实时、红点=离线/未认证），下面那一行同时承载**当前文件的路径**和它自己的动作（复制路径 / `@引用` / 全屏查看）以及**产物/文件树两个标签**——路径条和标签条原本各自占一整行，合并后那一行整行还给了文档本身，宽度按预览区实测宽度对齐，拖分隔条时会跟着走。弹出页上的 `@` 按钮会把引用直接写进主窗口的输入框；主窗口不在了才退回剪贴板。**它和侧边栏都默认落在文件树上**：文件树是“怎么找到文件”，台账是“智能体改了什么”——后者是之后才问的问题。把「文件树」开关关掉时，两边都会退回台账（没有树可开了）。
- **一键把正在看的文件送到弹出页。** 每个打开的文件都带一个 **在弹出页打开**，而且**从不占用自己的一行**：文件可编辑时它在编辑器自己的工具条里（那条本来就要有：预览/编辑/保存），不可编辑时它贴在**文件标签条**的右端——也就是文件上方那一行。系统侧边栏里，产品自己的标题行（路径 + 渲染器 + 刷新）没有任何扩展点，所以那里没有第二行可挤：能编辑的文件走编辑器工具条，其余文件（HTML、图片、Office、PDF、大到不能编辑的文本）走**文件标签的右键菜单**（`在弹出页打开 <文件名>`）——这也是唯一能覆盖本插件根本不渲染的文件的入口。文件树的右键菜单里也有同一项，任何文件都能用。点一下就把弹出页打开（或复用已有的那个标签页）并直接停在**这个文件**上，地址是按会话工作区补全的绝对路径，所以两边看到的是同一份文档。弹出页启动时会认 `?path=<文件>`，并在文件树里定位到它；指向的文件不存在时会在预览区说明原因，而不是悄悄退回产物列表。
- **弹出页的全屏预览。** 预览条上的 全屏 按钮把文档铺满整个屏幕：浏览器允许时走 Fullscreen API（没有浏览器边框、没有任务栏，正是把文档放到第二块屏幕时要的效果），API 缺失或被拒绝时（例如嵌在 iframe 里）退化为铺满当前标签页的 CSS 模式。两种模式都能用 `Esc` 退出，按钮也会如实报告当前状态（`aria-pressed`），包括浏览器自己改变状态的情况。
...
- **文件树的图标和系统那棵一致。** 用的是产品自带文件树的同一套图标：代码文件显示自己的语言彩色图标（`.js` / `.ts` / `.py` / `.rs` / `.go` / `package.json` 等 48 种），其余文件显示带类型标记的文件卡（MD / PDF / 表格 / 播放三角 / 图片，颜色按类型），文件夹用的是产品自己的文件夹图标。两张面（侧边栏面板与弹出页）取同一份图标表与同一套分类规则，所以同一个文件在哪个窗口里看都是同一个图标；那份 48 个品牌图标的绘制指令由 `scripts/gen-filetype-icons.js` 从**本机装着的 DSH** 里解出并生成，`npm run check` 会在它与装机版本对不上时失败，而不是悄悄留着旧图标。
- **标签页也戴同一套图标。** 弹出页那两个视图标签（**产物** / **文件树**）画的是产品自己用的那两枚 16-box 描边图标（产物 = 产品交付物行的「一摞纸」，文件树 = 产品「文件」标签那个线框文件夹）；面板里每个打开的文件标签显示**它自己的类型图标**——和文件树的行用的是同一个分类器，所以标签和打开它的那一行不会各说各话；原生侧边栏的引导页（「文件树 / 产物 / 任务 / 用量 / Git」那一列）每一行也带图，其中文件树那行用的就是产品 `GuideArtworkFiles` 的原图，和产品自己的「文件」条目并排时看不出差别。
...
- **编辑与保存。** Markdown、纯文本和 CSV 在面板、弹出页**和系统侧边栏自己的文档页**里都能编辑（后者的判定是“整个文件在一页里送到了”：系统按 5000 行一页投递，所以这个长度以内都能编辑，更长的文件请用「在弹出页打开」去弹出页里编辑）。如果保存前文件已被别人改动，会明确拒绝并给出选择，而不是悄悄覆盖；保存还会并入智能体改动的那份撤销历史，所以撤销也能撤掉你自己的修改。
...
| 设置项                       | 默认值           | 说明                                                                                                                                                                                                                                                                 |
| ------------------------------ | ------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 加载时展开                   | 开               | 页面加载完就打开面板。在原生侧边栏形态下，它还会为新建会话打开本插件的「文件树」页，一次完成展开并落到文件树上。你已经用过的会话不会被碰。                                                                                                                           |
| 自动刷新                     | 开               | 面板打开时每两秒拉取一次新产物。弹出页同样遵守这个开关，切回该标签页时也会刷新一次。                                                                                                                                                                                 |
| 文件树                       | 开               | 在浮动面板的标签条上显示文件树。原生侧边栏形态不提供这个开关：本插件的树在那一列里一定有一席之地（是哪一个标签，见下面两项）。                                                                                                                             |
| 用系统右侧边栏承载面板       | 开               | 把每个视图都注册成系统右侧边栏自己的标签页，展开/收起、全屏、拖宽、标签条都由系统提供。关掉之后面板退回浮动形态；系统的「文件」标签原样保留。刷新页面后生效。                                                                                |
| 显示系统的文件树             | 关               | **关（默认）**：右侧边栏只有一棵文件树——本插件的树接管系统「文件」这个位置（`@引用`、右键菜单、新建/删除、按目录刷新、A/M 字母都在），开始页也只有一条文件树条目。代价说清楚：系统的实时目录监听住在它自己的页面里，接管后一并停用，改由本插件在产物变化时刷新相应目录；另一个编辑器改动的文件需要手动刷新一次。**开**：系统自己的「文件」标签回到它的树（自带实时监听与自动刷新），本插件的树是旁边的「文件树」标签，两棵都在。刷新页面后生效。 |
...
- 针对 `web` profile 的 **DSH `0.1.7-rc.1`** 开发和测试，这是唯一带浏览器界面的 profile。
...
本插件最初 fork 自 Qinyun Cai 的 [e2mcc/dsh-popout-sidebar](https://github.com/e2mcc/dsh-popout-sidebar)，其上游许可声明在 [LICENSE](./LICENSE) 中未作改动地保留。此后它陆续长出了产物台账、工作区文件树、离线预览和 Office 阅读器、编辑与保存、只读 Git 面板，以及借回给系统的那些渲染器——最近一次是 Markdown 文档皮肤、原生 HTML（`<picture>` / 徽章图片链接）渲染、把选中内容直接引成「文件:行号」、把 `@引用` 挂上系统文档预览的工具栏，以及修掉「在系统侧边栏打开」因拿不到当前会话而失败的问题。
